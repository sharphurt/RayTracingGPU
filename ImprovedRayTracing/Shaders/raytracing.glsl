...
vec4 castRay(inout vec3 ro, inout vec3 rd) {
	...
	vec3 n;
	mat2x4 spheres[6];
	spheres[0][0] = vec4(-1, -1, -1.5, 0.5);
	spheres[1][0] = vec4(-1, 0, -1, 0.3);
...
/*	spheres[3][0] = vec4(3.5, -1.0, 0.5, 0.5);
	...
	spheres[1][1] = vec4(fromRGB(207, 160, 19), -1);
//	spheres[2][1] = vec4(1.0, 0.0, 0.5, 1.0);
	...
	/*
	vec3 lightBoxPos = vec3(-1, -1, -3.3);
	vec3 lightBoxSize = vec3(0.5);
	
	vec3 lightBoxN;
	
	it = boxIntersection(ro - lightBoxPos, rd, lightBoxSize, lightBoxN);
	if (it.x > 0.0 && it.x < minIt.x) {
		minIt = it;
		n = lightBoxN;
		col = vec4(1, 1, 1, -2);
	...
	*/
	...
	planes[1][1] = vec4(19 / 255.0, 15 / 255.0, 148 / 255.0, 0);
	planes[2][1] = vec4(19 / 255.0, 60 / 255.0, 173 / 255.0, 0);
